...
if [ ! -f /var/www/html/wp-config.php ]; then
	...
	wp core install --url=https://rlandolt.42.fr --title="inception" --admin_user="$(cat /run/secrets/wpmngr)" --admin_password="$(cat /run/secrets/dbpsw)" --admin_email=rlandolt@student.42lisboa.com --allow-root
